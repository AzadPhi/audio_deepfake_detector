...
def initialize_model_heavy(input_shape: tuple):
...
def compile_model_heavy(model: models.Model, learning_rate=0.0005):
## Compile the model
    model.compile(
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        model = initialize_model_heavy(X_train.shape[1:])

        model = compile_model_heavy(model)

        model, history = train_model_heavy(model, X_train, y_train, validation_data=(X_test, y_test))
    else:
        print("Not working Retry")
